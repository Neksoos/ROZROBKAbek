...
from routers.inventory import give_item_to_player  # якщо інший шлях — поправ
...
async def _consume_inventory_item(conn, tg_id: int, item_code: str, amount: int):
    # ✅ player_inventory зберігає item_id, але API працює з item_code
    item_id = await _item_code_to_id(conn, item_code)
    if not item_id:
        raise HTTPException(404, "ITEM_NOT_FOUND")

    row = await conn.fetchrow(
        "SELECT amount FROM player_inventory WHERE tg_id=$1 AND item_id=$2",
        tg_id, item_id
    )
    have = int(row["amount"]) if row else 0
    if have < amount:
        raise HTTPException(400, f"NOT_ENOUGH_ITEMS:{item_code}:{have}/{amount}")

    await conn.execute(
        """
        UPDATE player_inventory
        SET amount = amount - $3, updated_at = now()
        WHERE tg_id=$1 AND item_id=$2
        """,
        tg_id, item_id, amount
    )
    await conn.execute(
        "DELETE FROM player_inventory WHERE tg_id=$1 AND item_id=$2 AND amount <= 0",
        tg_id, item_id
    )
...
async def _add_inventory_item(conn, tg_id: int, item_code: str, amount: int):
    ...
    # ✅ потрібно щоб був unique (tg_id, item_id)
    await conn.execute(
        """
        INSERT INTO player_inventory(tg_id, item_id, amount, created_at, updated_at)
        VALUES ($1,$2,$3, now(), now())
        ON CONFLICT (tg_id, item_id)
        DO UPDATE SET amount = player_inventory.amount + EXCLUDED.amount, updated_at = now()
        """,
        tg_id, item_id, amount
    )


async def _add_player_material_by_code(conn, tg_id: int, material_code: str, qty: int):
    mid = await _material_code_to_id(conn, material_code)
    if not mid:
        raise HTTPException(400, f"MATERIAL_NOT_FOUND:{material_code}")
...
async def claim(queue_id: int, tg_id: int = Query(..., gt=0)):
    ...
    async with pool.acquire() as conn:
        ...
        item = await conn.fetchrow(
            "SELECT code, name, category, emoji, rarity, description, stats FROM items WHERE code=$1",
            q["output_item_code"],
        )
        if not item:
        ...
        async with conn.transaction():
            await give_item_to_player(
                ...
                amount=int(q["output_amount"] or 1),
            )
...
async def list_player_herbs(tg_id: int = Query(..., gt=0)):
    ...
    ✅ Інвентар: player_inventory має item_id.
    """
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch(
...
async def start_drying(req: DryingStartRequest):
    ...
    async with pool.acquire() as conn:
        ...
        # ✅ dried material мапа по item_code
        ...
        async with conn.transaction():
            existing = await conn.fetchrow(
                ...
                req.tg_id, req.slot_index
            )
            if existing and str(existing["status"]) in ("drying", "done"):
                raise HTTPException(400, "SLOT_ALREADY_USED")

            # ✅ списання з інвентарю через item_id (всередині хелпера)
            await _consume_inventory_item(conn, req.tg_id, req.item_code, req.amount)
...
async def claim_drying(slot_index: int, tg_id: int = Query(..., gt=0)):
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            await _add_player_material_by_code(
                conn,
                tg_id=int(row["tg_id"]),
                material_code=row["output_material_code"],
                qty=int(row["output_amount"]),
            )
...
async def cancel_drying(slot_index: int, tg_id: int = Query(..., gt=0)):
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            # ✅ повернення в інвентар через item_id (всередині хелпера)
