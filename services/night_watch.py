...
def medal_drop_chance(mob_level: int) -> float:
    ...
    ✅ ПІДНЯТО ВДВІЧІ (від попередньої формули):
      було: базово 1.0% + 0.4% за рівень, cap 20%
      стало: базово 2.0% + 0.8% за рівень, cap 40%
    ...
    Приклади:
      ...
      lvl 50 -> 40% (cap)
    """
    base, scale = 0.02, 0.008  # 2.0% + 0.8% * level  (x2)
    c = base + scale * max(1, mob_level)
    return min(max(c, 0.0001), 0.40)
